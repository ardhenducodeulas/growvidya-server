...
class AdminAcademicController {
    ...
    try {
      ...
      const teacherId =
        ...
        (String(req.user?.roleName || '').toLowerCase().includes('teacher')
          ...
          : null);
      const data = await AcademicModel.getClasses(schoolId, activeOnly, teacherId);
  ...
  static async createClass(req, res, next) {
    try {
      const schoolId = AdminAcademicController.getSchoolId(req);
      const insertId = await AcademicModel.createClass(schoolId, req.body);
  ...
  static async updateClass(req, res, next) {
    try {
      const schoolId = AdminAcademicController.getSchoolId(req);
      const updated = await AcademicModel.updateClass(req.params.id, schoolId, req.body);
  ...
  static async getSections(req, res, next) {
    try {
      ...
      const data = await AcademicModel.getSections(schoolId, classId, activeOnly, status);
  ...
  static async createSection(req, res, next) {
    try {
      const schoolId = AdminAcademicController.getSchoolId(req);
      const insertId = await AcademicModel.createSection(schoolId, req.body);
  ...
  static async getDocumentTypes(req, res, next) {
    try {
      const data = await AcademicModel.getDocumentTypes();
  ...
  static async getSyllabusById(req, res, next) {
    try {
      ...
      const { id } = req.params;
      const data = await AcademicModel.getSyllabusById(id, schoolId);
      if (!data) return ApiResponse.error(res, 'Syllabus record not found', null, 404);
  ...
  static async createSyllabus(req, res, next) {
    try {
      const schoolId = AdminAcademicController.getSchoolId(req);
      const { academic_year, academic_year_id, class_id, classId, subject_id, subjectId, lession, lesson, status } = req.body;
      const insertId = await AcademicModel.createSyllabus(schoolId, {
        academic_year: academic_year || academic_year_id,
        class_id: class_id || classId,
        subject_id: subject_id || subjectId,
        lession: lession || lesson,
        status: status !== undefined ? Number(status) : 1,
      });
      return ApiResponse.success(res, 'Syllabus created successfully', { id: insertId }, 201);
    } catch (error) {
      next(error);
    }
  }
  ...
  static async updateSyllabus(req, res, next) {
    try {
      ...
      const { id } = req.params;
      const { academic_year, academic_year_id, class_id, classId, subject_id, subjectId, lession, lesson, status } = req.body;
      const success = await AcademicModel.updateSyllabus(id, schoolId, {
        academic_year: academic_year || academic_year_id,
        class_id: class_id || classId,
        subject_id: subject_id || subjectId,
        lession: lession || lesson,
      ...
      if (!success) return ApiResponse.error(res, 'Syllabus not found or update failed', null, 400);
      return ApiResponse.success(res, 'Syllabus updated successfully');
    } catch (error) {
      next(error);
    }
  ...
  static async deleteSyllabus(req, res, next) {
    try {
      const schoolId = AdminAcademicController.getSchoolId(req);
      const { id } = req.params;
      const success = await AcademicModel.deleteSyllabus(id, schoolId);
      if (!success) return ApiResponse.error(res, 'Syllabus not found or delete failed', null, 400);
      return ApiResponse.success(res, 'Syllabus deleted successfully');
    } catch (error) {
      next(error);
    }
