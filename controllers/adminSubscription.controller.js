...
class AdminSubscriptionController {
  ...
  /**
   * Upgrade to a paid subscription plan (Direct fallback / administrative)
   */
  static async upgradeSubscription(req, res, next) {
    try {
      ...
      const { plan_id, planId, amount_paid, amountPaid, payment_gateway, paymentGateway, payment_transaction_id, paymentTransactionId } = req.body;
      const targetPlanId = plan_id || planId;
      if (!targetPlanId) {
      ...
      return ApiResponse.success(res, 'Subscription upgraded successfully! Your annual license is now active.', upgraded, 200);
    } catch (error) {
      next(error);
    }
  }
