...
class AdminFeesModel {
  ...
  static async getAllStructures({ schoolId, academicYearId, status, search }) {
    let query = `
      SELECT 
        fs.id,
        fs.school_id,
      ...
      if (id) {
        ...
        const insertQuery = `
          INSERT INTO fee_structures (
            school_id, name, class_id, section_id, academic_year_id, frequency,
  ...
  static async getAllocations({ schoolId, classId, sectionId, structureId, academicYearId, search, page, limit }) {
    let whereClause = `
      WHERE sfa.school_id = ?
        AND sfa.status != 4
    `;
      ...
      for (const st of targetStudents) {
        ...
        const [insRes] = await connection.query(
          `INSERT INTO fee_invoices (
             invoice_no, school_id, student_id, class_id, section_id,
  ...
  static async getCollectionStats(schoolId) {
    // Total collected
    const [collectedRes] = await pool.query(
      `SELECT COALESCE(SUM(amount_paid), 0) AS total_collected FROM fee_payments WHERE school_id = ?`,
      [schoolId]
    );
    ...
    const [invoiceRes] = await pool.query(
      `SELECT 
         ...
         COUNT(CASE WHEN status = 'Overdue' OR (status = 'Unpaid' AND due_date < CURDATE()) THEN 1 END) AS overdue_invoices
       FROM fee_invoices 
       WHERE school_id = ?`,
      [schoolId]
    );
    ...
    const [todayRes] = await pool.query(
      `SELECT COALESCE(SUM(amount_paid), 0) AS today_collected 
       FROM fee_payments 
       WHERE school_id = ? AND DATE(payment_date) = CURDATE()`,
      [schoolId]
    );

    // This month's collection
    const [monthRes] = await pool.query(
      `SELECT COALESCE(SUM(amount_paid), 0) AS month_collected 
       FROM fee_payments 
       WHERE school_id = ? 
  ...
  static async getAllPayments({ schoolId, studentId, invoiceId, paymentMethod, dateFrom, dateTo, search }) {
    let query = `
      SELECT 
        fp.id,
        fp.txn_no,
    ...
    try {
      ...
      const [payRes] = await connection.query(
        `INSERT INTO fee_payments (
           txn_no, receipt_no, school_id, student_id, invoice_id,
