...
class TeacherModel {
  static async getAllTeachers(schoolId, { search = '', email = '', status = '', limit = 12, offset = 0 } = {}) {
    let sql = `
      SELECT t.*, 
    ...
    const [rows] = await pool.query(
      `SELECT 
       ...
       LEFT JOIN blood_group_master bg ON (
         ...
         (t.blood_group NOT IN ('0', 0, '') AND CAST(t.blood_group AS CHAR) = bg.blood_group)
       )
       LEFT JOIN marital_master mm ON (
         CAST(t.marital_status AS CHAR) = CAST(mm.id AS CHAR) OR
         (t.marital_status NOT IN ('0', 0, '') AND CAST(t.marital_status AS CHAR) = mm.marital_status)
       )
  ...
  static async createTeacher(schoolId, data) {
    ...
    const [result] = await pool.query(
      `INSERT INTO teacher_master 
       (school_id, picture, academic_year, teacher_id, first_name, last_name, class, section, subject,
