...
class AcademicModel {
  ...
  static async getClasses(schoolId, activeOnly = false, teacherId = null) {
    let sql = `SELECT c.*, s.shift_name FROM class_master c 
  ...
  static async getClassById(id, schoolId) {
    const [rows] = await pool.query(
      `SELECT c.*, s.shift_name FROM class_master c 
       LEFT JOIN shift_master s ON c.shift_id = s.id 
  ...
  static async createClass(schoolId, { class_name, shift_id = null, sort_order = 0, status = 1 }) {
    const [result] = await pool.query(
      `INSERT INTO class_master (school_id, shift_id, class_name, sort_order, status) VALUES (?, ?, ?, ?, ?)`,
      [schoolId, shift_id, class_name, sort_order, status]
    );
    return result.insertId;
  }

  static async updateClass(id, schoolId, { class_name, shift_id, sort_order, status }) {
    const [result] = await pool.query(
      `UPDATE class_master SET class_name = ?, shift_id = ?, sort_order = ?, status = ? WHERE id = ? AND school_id = ?`,
      [class_name, shift_id, sort_order, status, id, schoolId]
    );
    return result.affectedRows > 0;
  }
  ...
  // ==================== SECTIONS ====================
  static async getSections(schoolId, classId = null, activeOnly = false, status = null) {
    let sql = `SELECT sec.*, c.class_name FROM section_master sec
  ...
  static async createSection(schoolId, { class_id, section_name, capacity = 40, note = '', sort_order = 1, status = 1 }) {
    const [result] = await pool.query(
      `INSERT INTO section_master (school_id, class_id, section_name, capacity, note, sort_order, status) VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [schoolId, class_id, section_name, Number(capacity) || 0, note || '', Number(sort_order) || 1, Number(status)]
    );
    return result.insertId;
  }
  ...
  static async getAssignments(schoolId, { classId, sectionId, subjectId, search, status, page, limit } = {}) {
    try {
      let baseSql = `
        FROM assignments a
