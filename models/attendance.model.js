...
class AttendanceModel {
   ...
   */
  static async getStudentAttendanceList(schoolId, { class_id, section_id, academic_year, date, page = 1, limit = 10, search = '' }) {
   ...
   */
  static async getStudentsForAttendance(schoolId, { class_id, section_id, academic_year, date, page = 1, limit = 10, search = '' }) {
    const formattedDate = String(date || '').split('T')[0].trim();
    const joinConditions = [
      `sa.student_id = s.id`,
      `(sa.date = ? OR DATE(sa.date) = ?)`,
   ...
   */
  static async saveStudentAttendance(schoolId, { attendanceDate, academic_year, attendanceRecords = [] }) {
    ...
    for (const record of attendanceRecords) {
      ...
      if (existing && existing.length > 0) {
        await pool.query(
          `UPDATE student_attendance SET attendance = ?, notes = ?, academic_year = ? WHERE id = ?`,
          [attendance, notes, academic_year || null, existing[0].id]
        );
      } else {
        await pool.query(
          `INSERT INTO student_attendance (school_id, student_id, attendance, academic_year, notes, date, status)
           VALUES (?, ?, ?, ?, ?, ?, 1)`,
          [schoolId, studentId, attendance, academic_year || null, notes, validDate]
        );
      }
    }
    return true;
   ...
   */
  static async getTeacherAttendanceList(schoolId, { date, page = 1, limit = 10, search = '' }) {
    const formattedDate = String(date || '').split('T')[0].trim();
    const joinConditions = [
      `ta.teacher_id = t.id`,
   ...
   */
  static async getTeachersForAttendance(schoolId, { date, page = 1, limit = 10, search = '' }) {
    const formattedDate = String(date || '').split('T')[0].trim();
    const joinConditions = [
      `ta.teacher_id = t.id`,
      `(ta.date = ? OR DATE(ta.date) = ?)`,
   ...
   */
  static async saveTeacherAttendance(schoolId, { attendanceDate, attendanceRecords = [] }) {
    ...
    for (const record of attendanceRecords) {
      ...
      if (existing && existing.length > 0) {
        await pool.query(
          `UPDATE teacher_attendance SET attendance = ?, notes = ? WHERE id = ?`,
          [attendance, notes, existing[0].id]
        );
      } else {
        await pool.query(
          `INSERT INTO teacher_attendance (school_id, teacher_id, attendance, notes, date, status)
           VALUES (?, ?, ?, ?, ?, 1)`,
          [schoolId, teacherId, attendance, notes, validDate]
        );
      }
    }
    return true;
   ...
   */
  static async getStaffAttendanceList(schoolId, { date, page = 1, limit = 10, search = '' }) {
    const formattedDate = String(date || '').split('T')[0].trim();
    const joinConditions = [
   ...
   */
  static async getStaffForAttendance(schoolId, { date, page = 1, limit = 10, search = '' }) {
    const formattedDate = String(date || '').split('T')[0].trim();
    const joinConditions = [
      `uma.user_master_id = u.id`,
      `(uma.date = ? OR DATE(uma.date) = ?)`,
   ...
   */
  static async saveStaffAttendance(schoolId, { attendanceDate, attendanceRecords = [] }) {
    ...
    for (const record of attendanceRecords) {
      ...
      if (existing && existing.length > 0) {
        await pool.query(
          `UPDATE user_master_attendance SET attendance = ?, notes = ? WHERE id = ?`,
          [attendance, notes, existing[0].id]
        );
      } else {
        await pool.query(
          `INSERT INTO user_master_attendance (school_id, user_master_id, attendance, notes, date, status)
           VALUES (?, ?, ?, ?, ?, 1)`,
          [schoolId, userId, attendance, notes, validDate]
        );
      }
    }
    return true;
